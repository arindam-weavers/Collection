...
function grant_theme_optionsframework_add_admin() {
    global $query_string;

    $themename = grant_theme_get_option('of_themename');
    $shortname = grant_theme_get_option('of_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = grant_theme_get_option('of_template');
            grant_theme_reset_options($options, 'optionsframework');
            header("Location: admin.php?page=optionsframework&reset=true");
            die;
        }
    }
    $of_page = add_theme_page($themename, 'Option Panel', 'edit_theme_options', 'optionsframework', 'grant_theme_optionsframework_options_page', 'div');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'grant_theme_load_only');
}

add_action('admin_menu', 'grant_theme_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */

function grant_theme_reset_options($options, $page = '') {
    global $wpdb;
    $count = 0;
    ...
    if ($page == 'optionsframework') {
        grant_theme_delete_option('of_options');
    }
}
...
function grant_theme_optionsframework_options_page() {
    $options = grant_theme_get_option('of_template');
    $themename = grant_theme_get_option('of_themename');
    ?>
    ...
    <?php wp_nonce_field('theme-update-option'); ?>
            <div id="header">
                <div class="logo">
                    <h2><?php _e('Option Panel'); ?></h2>
                </div>
               <div class="clear"></div>
            </div>
    <?php
    // Rev up the Options Machine
    $return = grant_theme_optionsframework_machine($options);
    ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
...
function grant_theme_load_only() {
    add_action('admin_head', 'grant_theme_admin_head');
    ...
    function grant_theme_admin_head() {
        // COLOR Picker 
        ?>
        <link rel="stylesheet" media="screen" type="text/css" href="<?php echo get_template_directory_uri(); ?>/functions/css/colorpicker.css" />
        <!--<script type="text/javascript" src="<?php echo get_template_directory_uri(); ?>/functions/js/jscolor.js"></script>-->
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = grant_theme_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = grant_theme_get_option($option_id);
                }
                ?>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - grant_theme_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action', 'grant_theme_ajax_callback');

function grant_theme_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        grant_theme_update_option($clickedID, $uploaded_file['url']);
        ...
        foreach ($options as $option_array) {
            $id = $option_array['id'];
            $old_value = grant_theme_get_option($id);
            $new_value = '';
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            grant_theme_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    grant_theme_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    grant_theme_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            grant_theme_update_option($multicheck_id, 'false');
                        } else {
                            grant_theme_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    grant_theme_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    grant_theme_update_option($id, $border_array);
...
function grant_theme_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = grant_theme_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'select':
                ...
                $select_value = grant_theme_get_option($value['id']);
            ...
            case "checkbox":
				...
				foreach ($value['options'] as $key => $option) {
                    ...
                    if (isset($val[$option])) {
					 ...
					 $stored = grant_theme_get_option($value['id']);
					
					 if($stored == 'false'){
						 
						 $output .= '<input id="' . esc_attr($id) . '" class="checkbox of-input" value="'.$value['std'].'" type="checkbox" name="' . esc_attr($name) . '" />';
						 
						 
						 
					 }else{
						 
						 $output .= '<input id="' . esc_attr($id) . '" class="checkbox of-input" checked="checked" value="'.$value['std'].'" type="checkbox" name="' . esc_attr($name) . '" />';
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= grant_theme_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":

                $output .= grant_theme_optionsframework_uploader_function($value['id'], $value['std'], 'min');
            ...
            case "color":
                ...
                $output .= '<input style="width:125px" class="color" name="' . $value['id'] . '" id="' . $value['id'] . '" type="color" value="' . $val . '" />';
				 //$output .= '<input id="bgColor" style="width:125px" class="color" name="bgColor" id="bgColor" type="text" value="#'.$val.'" />';
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = grant_theme_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = grant_theme_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                ...
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = grant_theme_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - grant_theme_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */

function grant_theme_optionsframework_uploader_function($id, $std, $mod) {
    ...
    $uploader = '';
    $upload = grant_theme_get_option($id);

    if ($mod != 'min') {
        $val = $std;
        if (grant_theme_get_option($id) != "") {
            $val = grant_theme_get_option($id);
        }
